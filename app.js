    ...
    const effectPresets = {
      cinematic: { contrast: 1.3, saturation: 0.9, temperature: 0.3, exposure: 0.9 },
      vibrant: { saturation: 1.8, vibrance: 0.5, exposure: 1.1, contrast: 1.2 },
      goldenHour: { temperature: 0.8, exposure: 1.2, vignette: 0.4, saturation: 1.3 },
      coolMoody: { temperature: -0.6, exposure: 0.8, contrast: 1.4, vibrance: -0.2 },
      instagram: { saturation: 1.5, contrast: 1.2, vignette: 0.2, exposure: 1.05 },
      youTube: { contrast: 1.3, saturation: 1.1, temperature: 0.2, exposure: 1.05 }
    };
    ...
    function playTimeline() {
      ...
      if (audioTracks.length > 0 && audioElement.src) {
        audioElement.currentTime = audioTracks[0].startTime;
        audioElement.play();
      }
      ...
      function renderTimelineFrame() {
        ...
        renderTextOnCanvas(currentTime);
    ...
    document.getElementById('audioVolume').addEventListener('input', (e) => {
      ...
      if (audioTracks.length > 0) {
        audioTracks[0].volume = parseFloat(e.target.value);
    ...
    function setTextPosition(position) {
      ...
      document.getElementById('textPositionDisplay').textContent =
    ...
    function addTextOverlay() {
      ...
      const textOverlay = {
        ...
        startTime: parseFloat(document.getElementById('textStart').value),
        duration: parseFloat(document.getElementById('textDuration').value)
      };

      textOverlays.push(textOverlay);

      updateTextStatus('Text added: "' + content + '"');
      renderTextList();
    ...
    function renderTextList() {
      ...
      textOverlays.forEach((text) => {
        ...
        div.style.borderLeftColor = text.color;

        const info = `"${text.content}" (${text.startTime.toFixed(1)}s-${(text.startTime + text.duration).toFixed(1)}s)`;
        const deleteBtn = document.createElement('button');
        deleteBtn.textContent = 'Delete';
        deleteBtn.onclick = () => deleteTextOverlay(text.id);
    ...
    function previewText() {
      ...
      updateTextStatus('Playing preview with text');
      playTimeline();
    }

    function renderTextOnCanvas(currentTime) {
      textOverlays.forEach(text => {
        const endTime = text.startTime + text.duration;

        if (currentTime >= text.startTime && currentTime <= endTime) {
          drawText(
            text.content,
            text.position,
            text.color,
    ...
    function updateTextStatus(message) {
    ...
    function addAudioTrack() {
      ...
      const audioTrack = {
        ...
        startTime: parseFloat(document.getElementById('audioStart').value)
      };

      audioTracks = [audioTrack];

      updateAudioStatus('Audio track added');
      renderAudioList();
    ...
    function deleteAudioTrack(id) {
      ...
      updateAudioStatus('Audio track removed');
      renderAudioList();
    }
    ...
    function renderAudioList() {
      ...
      audioTracks.forEach((track) => {
        ...
        div.style.borderLeftColor = '#00d9ff';

        const info = `Audio (start: ${track.startTime.toFixed(1)}s, vol: ${Math.round(track.volume * 100)}%)`;
        const deleteBtn = document.createElement('button');
        deleteBtn.textContent = 'Delete';
        deleteBtn.onclick = () => deleteAudioTrack(track.id);
    ...
    function previewAudio() {
      if (audioTracks.length === 0) {
        alert('Add audio first');
        return;
      }

      updateAudioStatus('Playing preview with audio');
      playTimeline();
    }
    ...
    function playExportTimeline() {
      ...
      if (audioTracks.length > 0 && audioElement.src) {
        audioElement.currentTime = audioTracks[0].startTime;
        audioElement.volume = audioTracks[0].volume;
        audioElement.play();
      }
      ...
      reader.onload = (e) => {
        try {
          ...
          video.addEventListener('loadedmetadata', () => {
            ...
            updateProjectStatus('Project loaded: ' + project.effects.length + ' effects restored');
    ...
    function newProject() {
      if (confirm('Create new project? (Current progress will be cleared)')) {
        timelineEffects = [];
        textOverlays = [];
        audioTracks = [];
        selectedEffectIndex = null;
